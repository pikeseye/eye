...
def build_model(input_shape):
    ...
    """ Decoder """
    d1 = decoder_block(b1, s4, input_shape[0]*2)                               ## 32
    d2 = decoder_block(d1, s3, input_shape[0])                                 ## 64
    d3 = decoder_block(d2, s2, input_shape[0]/2)                               ## 128
    d4 = decoder_block(d3, s1, input_shape[0]/4)                               ## 256

    """ Outputs """
    outputs = Conv2D(1, 1, padding="same", activation="sigmoid")(d4)

    model = Model(inputs, outputs, name="EffNetB0_UNET")
    return model
